...
const siteUrl = () => { const value=(process.env.NEXT_PUBLIC_SITE_URL || "https://milink.ca").replace(/\/$/, ""); return /^https:\/\//i.test(value) ? value : `https://${value.replace(/^https?:\/\//i, "")}`; };
...
export async function POST(request) {
  ...
  try {
    ...
    const logoUrl = `${siteUrl()}/Logo-Navy1.png`;
