...
export function Reveal({
  ...
  const visible = { opacity: 1, x: 0, y: 0, transition: { duration: durationIn, ease, delay } };
  const hidden = { opacity: fade ? 0 : 1, x: initX * 0.7, y: initY * 0.7, transition: { duration: durationOut, ease } };
  ...
  return (
    <motion.div
      ...
      className={["will-change-transform will-change-opacity", className].filter(Boolean).join(" ")}
      {...rest}
    >
      {children}
    </motion.div>
...
export function RevealStagger({
  ...
  const child = {
    ...
    exit: { opacity: itemFade ? 0 : 1, x: ix * 0.7, y: iy * 0.7, transition: { duration: 0.4, ease: "easeIn" } },
  };
  ...
  return (
    ...
    >
      {React.Children.map(children, (childNode) => {
        ...
        return (
          <motion.div variants={child} className="will-change-transform will-change-opacity">
            {childNode}
          </motion.div>
        );
      })}
...
export function SectionHeader({ eyebrow, title, subtitle, align = "center", className }) {
  return (
    <div className={["mx-auto max-w-3xl", align === "left" ? "text-left" : "text-center", className].filter(Boolean).join(" ")}>
      {eyebrow && (
