...
- Mobile dashboard standards: the sidebar becomes an auto-closing slide-over drawer; all navigation and account/support targets keep a minimum 44px touch target. The Admin Clients filters stay in one horizontally swipeable, snap-aligned row. Each client is a deliberately structured compact card: a single avatar/name/email identity block, a separate lifecycle-stage row, then health/project-count/joined metadata; it never reuses desktop absolute-positioned labels. Only the inline Message icon is exposed in the compact row; tapping the identity, status, or metadata area opens the inspector, where destructive delete remains behind confirmation. Payment actions wrap rather than overflow, and chat composers remain reachable while scrolling.
...
- A selected client yields project/profile detail, requirement context, assets, file-request review, chat shortcut, stage controls, proposal controls, payment controls, and destructive deletion. The lifecycle stage control is a dashboard-owned anchored picker (not a browser-native select): it opens directly beneath its trigger and above adjacent project-tab/action layers, keeps the adjacent Update stage action available, and stays within the mobile inspector card. When there are multiple projects, a compact stage-badged project tab strip selects the precise project; the inspector, requests, assets, approvals, timeline, handoff, and payments follow that selected project. Support chat remains intentionally account-wide.
...
- Admin inbox has client conversation list/search, unread markers, active thread, attachments, and composer. Every client has one unified support thread across all of their projects; the latest message's project may be shown as context, but it never splits or filters the conversation. The conversation header is intentionally focused on the client and their stage; it does not repeat an “Open Action Center” shortcut already available in navigation.
- **Admin Messages mobile behavior:** this is a deliberate two-state inbox, not a squeezed desktop split pane. The default phone view is the full client list with search and “Start new message.” Selecting a client replaces it with a full-height conversation with a 44px back control. The header and composer stay pinned while only the history scrolls; returning with Back restores the same list state.
...
- **Mobile:** sidebar becomes drawer; Admin Messages uses its list-first then full-conversation drill-down pattern. The Clients Directory deliberately swaps its six-column desktop table for a dedicated full-width client-card layout: identity and email remain readable beside a single 44px message action, while stage, project count, health, and joined date form a wrapping second row. Destructive actions remain in the inspector. Filters and pipelines use contained horizontal scrolling rather than causing page-level overflow. These are intentional responsive patterns, not desktop tables squeezed into a narrow viewport.
